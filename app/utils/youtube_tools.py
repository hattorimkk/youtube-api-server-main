...
class YouTubeTools:
    ...
    def get_video_captions(url: str, languages: Optional[List[str]] = None) -> str:
        ...
        try:
            ...
            # Đã sửa: Truy cập thuộc tính `.text` thay vì dùng `['text']`
            ...
            if script_text:
                return script_text
            return "No captions found for video in specified languages."
        except Exception as e:
    ...
    def get_video_timestamps(url: str, languages: Optional[List[str]] = None) -> List[str]:
        ...
        try:
            ...
            # Đã sửa: Truy cập thuộc tính `.start` và `.text`
            captions = ytt_api.fetch(video_id, languages=languages or ["en"])
            
            timestamps = []
            for line in captions:
                start = int(line.start) # Truy cập thuộc tính .start
                minutes, seconds = divmod(start, 60)
                timestamps.append(f"{minutes}:{seconds:02d} - {line.text}") # Truy cập thuộc tính .text
            return timestamps
        except Exception as e:
